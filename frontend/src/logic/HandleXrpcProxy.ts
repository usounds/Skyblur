...
import { AuthorizationServerMetadata } from '@/types/types';
...
export async function fetchWithDpop<T = unknown>(
  ...
  const accessToken = await getAccessToken(oauthKey);
...
export async function getAccessToken(oauthKey: string) {
...
export async function verifyCookie(signedValue: string) {
  const secret = getCloudflareContext().env.AUTH_SECRET;
  const [value, signatureB64] = signedValue.split('.');
  if (!value || !signatureB64) return null;
