...
export const ChangeModeModal: React.FC<ChangeModeModalProps> = ({ onConfirm, onClose }) => {
  ...
  const customTheme: PartialReablocksTheme = {
    components: {
      dialog: {
        ...
        inner:
          "flex flex-col box-border outline-0 pointer-events-auto overflow-auto max-w-screen-md bg-panel text-text-primary border border-panel-accent rounded shadow-2xl",
        header: {
  ...
  return (
    <ThemeProvider theme={extendTheme(theme, customTheme)}>
      <Dialog open={open} onClose={handleJustClose} header={locale.CreatePost_ChangeSimpleMode} size="90%">
        {() => (
          <>
            <div className="mb-6 text-gray-700">{locale.CreatePost_ChangeSimpleModeDescription}</div>
